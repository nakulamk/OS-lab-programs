...
pro P[50], R[50];
int NoOfProcess = 0, rn = 0, cputime = 0, startRq = 0;
...
int main()
{
    ...
    do
    {
        ...
        if (rn != NoOfProcess && rn != 0)
            preemtive(R);

        else if (rn == NoOfProcess)
            non_pre(R);

        cputime++;
    } while (startRq < NoOfProcess || cputime <= maxAT);
    ...
    avgTT = (float)totalTT / NoOfProcess;
    avgWT = (float)totalWT / NoOfProcess;
